        ...
        for i in range(h - 2):
            for j in range(w - 2):
                ...
                if sub_max != sub_min:
                    ...
                    pix1 = self.S[idx1]
                    pix2 = self.S[idx2]
                    diss1 = np.exp(-0.2*np.sum(np.square(pix1 - pix2)) / sigma ** 2) # 使用欧式距离来得到初始的邻接矩阵
                    diss2 = np.exp(-0.2*np.sum(np.square((idx1 - idx2)) ) / sigma ** 2)
                    a1_pix1 = np.exp(-0.2*np.sum(np.square(self.S[idx1-1]-pix1)) / sigma ** 2)
                    b3_pix2 = np.exp(-0.2*np.sum(np.square(self.S[idx2-1]-pix2)) / sigma ** 2)
                    sum_pix1 = a1_pix1/15
                    sum_pix2 = b3_pix2/15
                    u1_pix1 = ((a1_pix1/sum_pix1)*(self.S[idx1-1]))*(pix1)
                    u2_pix2 = ((b3_pix2/sum_pix2)*(self.S[idx2-1]))*(pix2)
                    diss3 = np.exp(-0.2*np.sum(np.square(u1_pix1 - u2_pix2)) / sigma ** 2)
...
class LDA_SLIC(object):
    ...
    def LDA_Process(self,curr_labels):
        ...
        lda.fit(x,y-1)
        X_new = lda.transform(self.x_flatt) # [145*145,15]
